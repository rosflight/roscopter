...
public:
  ...
  Xform operator+(const Vec6 & v) const { return boxplus(v); }
  ...
  {
    static_assert(Derived::RowsAtCompileTime == 3, "Can only transform 3x1 vectors");
    static_assert(Derived::ColsAtCompileTime == 1, "Can only transform 3x1 vectors");
...
{
  os << "t: [ " << X.t_(0, 0) << ", " << X.t_(1, 0) << ", " << X.t_(2, 0) << "] q: [ " << X.q_.w()
     << ", " << X.q_.x() << "i, " << X.q_.y() << "j, " << X.q_.z() << "k]";
  return os;
}
