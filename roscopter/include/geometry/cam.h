...
public:
  ...
  void pix2intrinsic(const Eigen::Matrix<T2, 2, 1> & pix, Eigen::Matrix<T2, 2, 1> & pi) const
  {
    ...
    pi << (1.0 / fx) * (pix.x() - cx - (s_ / fy) * (pix.y() - cy)), (1.0 / fy) * (pix.y() - cy);
  }
  ...
  void intrinsic2pix(const Eigen::Matrix<T2, 2, 1> & pi, Eigen::Matrix<T2, 2, 1> & pix) const
  {
    ...
    const T cx = cam_center_.x();
    const T cy = cam_center_.y();
    pix << fx * pi.x() + s_ * pi.y() + cx, fy * pi.y() + cy;
  }
  ...
  template<typename T2>
  void invProj(const Eigen::Matrix<T2, 2, 1> & pix, const T2 & depth,
               Eigen::Matrix<T2, 3, 1> & pt) const
  {
  ...
  Eigen::Matrix<T, 3, 3> K() const
  {
    ...
    out << focal_len_(0), s_, cam_center_(0), 0, focal_len_(1), cam_center_(1), 0, 0, 1;
    return out;
  }
...
public:
  ...
  void distortJac(const Vec2 & pi_u, Mat2 & J) const
  {
    ...
    Mat2 J1, J2;
    J1 << drdx * (x - dx), drdy * (x - dx), drdx * (y - dy), drdy * (y - dy);

    J2 << (1 - ddxdx), ddxdy, ddydx, (1 - ddydy);
  ...
  template<typename T2>
  void invProj(const Eigen::Matrix<T2, 2, 1> & pix, const T & depth,
               Eigen::Matrix<T2, 3, 1> & pt) const
  {
