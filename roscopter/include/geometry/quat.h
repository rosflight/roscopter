...
public:
  ...
  Quat(const T & roll, const T & pitch, const T & yaw)
      : arr_(buf_)
  {
    T cp = cos(roll / 2.0);
    T ct = cos(pitch / 2.0);
    ...
    arr_ << cp * ct * cs + sp * st * ss, sp * ct * cs - cp * st * ss, cp * st * cs + sp * ct * ss,
      cp * ct * ss - sp * st * cs;
  }
  ...
  static Quat from_R(const Eigen::Matrix<T, 3, 3> & m)
  {
    ...
    if (tr > 0) {
      T S = sqrt(tr + 1.0) * 2.;
      q.arr_ << 0.25 * S, (m(1, 2) - m(2, 1)) / S, (m(2, 0) - m(0, 2)) / S, (m(0, 1) - m(1, 0)) / S;
    } else if ((m(0, 0) > m(1, 1)) && (m(0, 0) > m(2, 2))) {
      T S = sqrt(1.0 + m(0, 0) - m(1, 1) - m(2, 2)) * 2.;
      q.arr_ << (m(1, 2) - m(2, 1)) / S, 0.25 * S, (m(1, 0) + m(0, 1)) / S, (m(2, 0) + m(0, 2)) / S;
    } else if (m(1, 1) > m(2, 2)) {
      T S = sqrt(1.0 + m(1, 1) - m(0, 0) - m(2, 2)) * 2.;
      q.arr_ << (m(2, 0) - m(0, 2)) / S, (m(1, 0) + m(0, 1)) / S, 0.25 * S, (m(2, 1) + m(1, 2)) / S;
    } else {
      T S = sqrt(1.0 + m(2, 2) - m(0, 0) - m(1, 1)) * 2.;
      q.arr_ << (m(0, 1) - m(1, 0)) / S, (m(2, 0) + m(0, 2)) / S, (m(2, 1) + m(1, 2)) / S, 0.25 * S;
    }
    return q;
  }
  ...
  static Quat from_euler(const T & roll, const T & pitch, const T & yaw)
  {
    ...
    Quat q;
    q.arr_ << cp * ct * cs + sp * st * ss, sp * ct * cs - cp * st * ss, cp * st * cs + sp * ct * ss,
      cp * ct * ss - sp * st * cs;
    return q;
  }
  ...
  Eigen::VectorBlock<const Eigen::Map<Vec4>, 3> bar() const { return arr_.template segment<3>(1); }
  ...
  Eigen::Matrix<T, 3, 1> uvec() const { return inverse().rotp(e3.cast<T>()); }

  // Get projection from 2D space orthogonal to unit vector
  Eigen::Matrix<T, 3, 2> proj() const { return inverse().R() * I_3x2.cast<T>(); }
