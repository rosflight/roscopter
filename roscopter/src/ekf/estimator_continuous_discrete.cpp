...
Eigen::Matrix3d skew_matrix(Eigen::Vector3d vec)
{
  ...
  skew_symmetric_matrix << 0.0, -vec(2), vec(1), vec(2), 0.0, -vec(0), -vec(1), vec(1), 0.0;
...
void EstimatorContinuousDiscrete::prediction_step(const Input & input)
{
  ...
  Eigen::Vector<double, num_estimator_inputs>
    imu_measurements; // These are the input to the propagation.
  imu_measurements << input.accel_x, input.accel_y, input.accel_z, input.gyro_x, input.gyro_y,
    input.gyro_z;

  std::tie(P_, xhat_) = propagate_model(xhat_, dynamics_model, jacobian_model, imu_measurements,
                                        input_jacobian_model, P_, Q_, Q_inputs_, Ts);
...
void EstimatorContinuousDiscrete::mag_measurement_update_step(const Input & input)
{
  ...
  std::tie(P_, xhat_) = measurement_update(xhat_, mag_info, tilt_mag_measurement_model, y_heading,
                                           tilt_mag_measurement_jacobian_model,
                                           tilt_mag_measurement_sensor_noise_model, P_);
}
...
void EstimatorContinuousDiscrete::baro_measurement_update_step(const Input & input)
{
  ...
  std::tie(P_, xhat_) =
    measurement_update(xhat_, _, baro_measurement_model, y_baro, baro_measurement_jacobian_model,
                       baro_measurement_sensor_noise_model, P_);
}
...
void EstimatorContinuousDiscrete::gnss_measurement_update_step(const Input & input)
{
  ...
  if (input.gps_new && gps_init_) {
    ...
    std::tie(P_, xhat_) =
      measurement_update(xhat_, _, gnss_measurement_model, y_gps, gnss_measurement_jacobian_model,
                         gnss_measurement_sensor_noise_model, P_);
  }
}

// ======== PREDICITON STEP EQUATIONS ========
// These are passed by reference to the predition step.
Eigen::VectorXd EstimatorContinuousDiscrete::dynamics(const Eigen::VectorXd & state,
                                                      const Eigen::VectorXd & inputs)
{
  ...
  Eigen::Vector3d velocity_dot = R(Theta).transpose() * gravity * Eigen::Vector3d::UnitZ() + y_accel
    + vels.cross(y_gyro - biases);
...
Eigen::MatrixXd EstimatorContinuousDiscrete::jacobian(const Eigen::VectorXd & state,
                                                      const Eigen::VectorXd & inputs)
{
...
Eigen::MatrixXd
EstimatorContinuousDiscrete::input_jacobian(const Eigen::VectorXd & state,
                                            [[maybe_unused]] const Eigen::VectorXd & inputs)
{
  // This uses both the accel and gyro. The associated jacobians have been combined.
  Eigen::Matrix<double, num_states, 6> G =
    Eigen::Matrix<double, num_states, num_estimator_inputs>::Zero();
...
{
  Eigen::Vector<double, num_tilt_mag_measurements> h =
    Eigen::Vector<double, num_tilt_mag_measurements>::Zero();
...
{
  Eigen::Matrix<double, num_tilt_mag_measurements, num_states> C =
    Eigen::Matrix<double, num_tilt_mag_measurements, num_states>::Zero();
...
Eigen::MatrixXd
EstimatorContinuousDiscrete::tilt_mag_measurement_sensor_noise(const Eigen::VectorXd & state,
                                                               const Eigen::VectorXd & input)
{
  Eigen::Matrix<double, num_tilt_mag_measurements, num_tilt_mag_measurements> R;

  Eigen::Matrix<double, num_tilt_mag_measurements, num_states> G_state =
    del_tilt_mag_del_states(input, state);

  Eigen::Matrix<double, num_tilt_mag_measurements, num_mag_measurements> G_mag =
    del_tilt_mag_del_mag(input, state);

  Eigen::Matrix<double, num_tilt_mag_measurements, num_states> C =
    tilt_mag_measurement_jacobian(input, state);

  R = G_mag * R_mag_ * G_mag.transpose() + G_state * P_ * G_state.transpose()
    - 2 * G_state * P_ * C.transpose() + R_tilt_;

  return R;
}

Eigen::MatrixXd EstimatorContinuousDiscrete::del_tilt_mag_del_mag(const Eigen::VectorXd & y_mag,
                                                                  const Eigen::VectorXd & state)
{
  ...
  double den = pow(my * c_phi - mz * s_phi, 2)
    + pow(mx * c_theta + my * s_phi * s_theta + mz * s_theta * c_phi, 2);
...
Eigen::MatrixXd EstimatorContinuousDiscrete::del_tilt_mag_del_states(const Eigen::VectorXd & y_mag,
                                                                     const Eigen::VectorXd & state)
{
  double mx = y_mag(2);
  double my = y_mag(3);
  ...
  Eigen::Matrix<double, num_tilt_mag_measurements, num_states> G =
    Eigen::Matrix<double, num_tilt_mag_measurements, num_states>::Zero();
  ...
  G(0, 6) =
    (mx * my * s_phi * c_theta + mx * mz * c_phi * c_theta + my * my * s_theta + mz * mz * s_theta)
    / den;
  G(0, 7) =
    (my * c_phi - mz * s_phi) * (-mx * s_theta + my * s_phi * c_theta + mz * c_phi * c_theta) / den;
  ...
  const Eigen::VectorXd & state, [[maybe_unused]] const Eigen::VectorXd & input)
{
  ...
  Eigen::Vector<double, num_baro_measurements> h =
    Eigen::Vector<double, num_baro_measurements>::Zero();
  ...
  [[maybe_unused]] const Eigen::VectorXd & state, [[maybe_unused]] const Eigen::VectorXd & input)
{
  ...
  Eigen::Matrix<double, num_baro_measurements, num_states> C =
    Eigen::Matrix<double, num_baro_measurements, num_states>::Zero();
...
Eigen::VectorXd EstimatorContinuousDiscrete::gnss_measurement_prediction(
  const Eigen::VectorXd & state, [[maybe_unused]] const Eigen::VectorXd & input)
{
  Eigen::Vector<double, num_gnss_measurements> h =
    Eigen::Vector<double, num_gnss_measurements>::Zero();
...
Eigen::MatrixXd EstimatorContinuousDiscrete::gnss_measurement_jacobian(
  const Eigen::VectorXd & state, [[maybe_unused]] const Eigen::VectorXd & input)
{
  Eigen::Matrix<double, num_gnss_measurements, num_states> C =
    Eigen::Matrix<double, num_gnss_measurements, num_states>::Zero();
  Eigen::Vector3d vels = state.block<3, 1>(3, 0);
  Eigen::Vector3d Theta = state.block<3, 1>(6, 0);
...
Eigen::Matrix3d EstimatorContinuousDiscrete::R(const Eigen::Vector3d & Theta)
{
  ...
  R_theta << cos(psi) * cos(theta), sin(phi) * sin(theta) * cos(psi) - sin(psi) * cos(phi),
    sin(phi) * sin(psi) + sin(theta) * cos(phi) * cos(psi), sin(psi) * cos(theta),
    sin(phi) * sin(psi) * sin(theta) + cos(phi) * cos(psi),
    -sin(phi) * cos(psi) + sin(psi) * sin(theta) * cos(phi), -sin(theta), sin(phi) * cos(theta),
    cos(phi) * cos(theta);
  return R_theta;
}
...
Eigen::Matrix3d EstimatorContinuousDiscrete::S(const Eigen::Vector3d & Theta)
{
  ...
  S_theta << 1.0, sin(phi) * tan(theta), cos(phi) * tan(theta), 0.0, cos(phi), -sin(phi), 0.0,
    sin(phi) / cos(theta), cos(phi) / cos(theta);
  return S_theta;
}

Eigen::Matrix3d EstimatorContinuousDiscrete::del_S_Theta_del_Theta(const Eigen::Vector3d & Theta,
                                                                   const Eigen::Vector3d & biases,
                                                                   const Eigen::Vector3d & gyro)
{
  ...
  S_Theta_jacobian << ((-bias_y + gyro_y) * cos(phi) + (bias_z - gyro_z) * sin(phi)) * tan(theta),
    ((-bias_y + gyro_y) * sin(phi) + (-bias_z + gyro_z) * cos(phi)) / (cos(theta) * cos(theta)),
    0.0, (bias_y - gyro_y) * sin(phi) + (bias_z - gyro_z) * cos(phi), 0.0, 0.0,
    ((-bias_y + gyro_y) * cos(phi) + (bias_z - gyro_z) * sin(phi)) * (1.0 / cos(theta)),
    ((-bias_y + gyro_y) * sin(phi) + (-bias_z + gyro_z) * cos(phi)) * tan(theta)
    * (1.0 / cos(theta)),
    0.0;
...
EstimatorContinuousDiscrete::del_R_Theta_T_g_del_Theta(const Eigen::Vector3d & Theta,
                                                       const double & gravity)
{
  ...
  R_theta_T_g_jac << 0.0, -gravity * cos(theta), 0.0, gravity * cos(phi) * cos(theta),
    -gravity * sin(phi) * sin(theta), 0.0, -gravity * sin(phi) * cos(theta),
    -gravity * sin(theta) * cos(phi), 0.0;

  return R_theta_T_g_jac;
}

Eigen::Matrix<double, 3, 3>
EstimatorContinuousDiscrete::del_R_Theta_v_del_Theta(const Eigen::Vector3d & Theta,
                                                     const Eigen::Vector3d & vels)
{
  double phi = Theta(0);
  double theta = Theta(1);
  double psi = Theta(2);
  ...
  R_theta_v_jac << v_e * (sin(phi) * sin(psi) + sin(theta) * cos(phi) * cos(psi))
      + v_d * (-sin(phi) * sin(theta) * cos(psi) + sin(psi) * cos(phi)),
    (-v_n * sin(theta) + v_e * sin(phi) * cos(theta) + v_d * cos(phi) * cos(theta)) * cos(psi),
    -v_n * sin(psi) * cos(theta) + v_e * (-sin(phi) * sin(psi) * sin(theta) - cos(phi) * cos(psi))
    + v_d * (sin(phi) * cos(psi) - sin(psi) * sin(theta) * cos(phi)),
    v_e * (-sin(phi) * cos(psi) + sin(psi) * sin(theta) * cos(phi))
    + v_d * (-sin(phi) * sin(psi) * sin(theta) - cos(phi) * cos(psi)),
    (-v_n * sin(theta) + v_e * sin(phi) * cos(theta) + v_d * cos(phi) * cos(theta)) * sin(psi),
    v_n * cos(psi) * cos(theta) + v_e * (sin(phi) * sin(theta) * cos(psi) - sin(psi) * cos(phi))
    + v_d * (sin(phi) * sin(psi) + sin(theta) * cos(phi) * cos(psi)),
    (v_e * cos(phi) - v_d * sin(phi)) * cos(theta),
    -v_n * cos(theta) - v_e * sin(phi) * sin(theta) - v_d * sin(theta) * cos(phi), 0.0;
...
Eigen::Vector3d
EstimatorContinuousDiscrete::calculate_inertial_magnetic_field(const double & declination,
                                                               const double & inclination)
{
  ...
  mag_inclination_rotation =
    Eigen::AngleAxisd(inclination, Eigen::Vector3d::UnitY()).toRotationMatrix();

  Eigen::Matrix3d mag_declination_rotation;
  mag_declination_rotation =
    Eigen::AngleAxisd(declination, Eigen::Vector3d::UnitZ()).toRotationMatrix();
...
void EstimatorContinuousDiscrete::calc_mag_field_properties(const Input & input)
{
  ...
  if ((0. <= inclination && inclination <= 90.) && (-90. <= declination && declination <= 90.)) {
  ...
  int mag_success = geomag_calc(input.gps_alt / 1000.0, input.gps_lat, input.gps_lon, decimal_year,
                                &declination_, &inclination_, &total_intensity, &grid_variation);
...
void EstimatorContinuousDiscrete::initialize_process_noises()
{
  ...
  Eigen::Vector<double, num_estimator_inputs> estimator_input_process_noises =
    Eigen::Vector<double, num_estimator_inputs>::Zero();
  estimator_input_process_noises << pow(accel_process_noise, 2), pow(accel_process_noise, 2),
    pow(accel_process_noise, 2), pow(radians(gyro_process_noise), 2),
    pow(radians(gyro_process_noise), 2), pow(radians(gyro_process_noise), 2);
...
void EstimatorContinuousDiscrete::bind_functions()
{
  ...
  gnss_measurement_sensor_noise_model =
    std::bind(&This::gnss_measurement_sensor_noise, this, _1, _2);

  tilt_mag_measurement_model = std::bind(&This::tilt_mag_measurement_prediction, this, _1, _2);
  tilt_mag_measurement_jacobian_model =
    std::bind(&This::tilt_mag_measurement_jacobian, this, _1, _2);
  tilt_mag_measurement_sensor_noise_model =
    std::bind(&This::tilt_mag_measurement_sensor_noise, this, _1, _2);
  ...
  baro_measurement_sensor_noise_model =
    std::bind(&This::baro_measurement_sensor_noise, this, _1, _2);
}
