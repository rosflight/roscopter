...
EstimatorEKF::kalman_update(Eigen::VectorXd x, Eigen::VectorXd h, Eigen::VectorXd y,
                            Eigen::MatrixXd C, Eigen::MatrixXd R, Eigen::MatrixXd S,
                            Eigen::MatrixXd P)
...
std::tuple<Eigen::MatrixXd, Eigen::VectorXd>
EstimatorEKF::measurement_update(Eigen::VectorXd x, Eigen::VectorXd inputs,
                                 MeasurementModelFuncRef measurement_model, Eigen::VectorXd y,
                                 JacobianFuncRef measurement_jacobian,
                                 SensorNoiseFuncRef sensor_noise_model, Eigen::MatrixXd P)
{
...
std::tuple<Eigen::MatrixXd, Eigen::VectorXd>
EstimatorEKF::propagate_model(Eigen::VectorXd x, DynamicModelFuncRef dynamic_model,
                              JacobianFuncRef jacobian, Eigen::VectorXd inputs,
                              JacobianFuncRef input_jacobian, Eigen::MatrixXd P, Eigen::MatrixXd Q,
                              Eigen::MatrixXd Q_g, double Ts)
{
  ...
  for (int _ = 0; _ < N; _++) {
    ...
    Eigen::MatrixXd A_d =
      Eigen::MatrixXd::Identity(A.rows(), A.cols()) + Ts / N * A + pow(Ts / N, 2) / 2.0 * A * A;
...
std::tuple<Eigen::MatrixXd, Eigen::VectorXd> EstimatorEKF::single_measurement_update(
  double measurement, double measurement_prediction, double measurement_variance,
  Eigen::VectorXd measurement_jacobian, Eigen::VectorXd x, Eigen::MatrixXd P)
{
  Eigen::MatrixXd I(x.size(), x.size());
  I = Eigen::MatrixXd::Identity(x.size(), x.size());
  Eigen::VectorXd L = (P * measurement_jacobian)
...
std::tuple<Eigen::MatrixXd, Eigen::VectorXd> EstimatorEKF::partial_measurement_update(
  ...
  Eigen::MatrixXd P, Eigen::VectorXd gammas)
{
  ...
  P = (gamma_outer_product.array() * P.array()).matrix()
    + ((ones_matrix - gamma_outer_product).array() * P_update.array()).matrix();
