...
class Mocap2Ublox:
    def __init__(self, Ts, gha, gva, gsa, no, rl, srp, srv, lo, A, B):
        ...
        self.ned_std_dev_3d = [
            self.global_horizontal_accuracy,
            self.global_horizontal_accuracy,
            self.global_vertical_accuracy,
        ]
        self.vel_std_dev_3d = [
    ...
    def update_rover_virtual_PosVelEcef(self, dt):
        ...
        self.rover_ned_noise = self.add_gps_noise(
            self.white_noise_3d, self.rover_ned_noise, self.sigma_rover_pos
        )
        ...
        self.rover_vel_lpf = self.lpf(
            self.rover_vel_noise, self.rover_vel_noise_prev, self.Ts, self.sigma_rover_vel
        )
    ...
    def Rx(self, theta):
        ...
        st = np.sin(theta)
        ct = np.cos(theta)
        rotx = np.array([[1.0, 0.0, 0.0], [0.0, ct, st], [0.0, -st, ct]])
    ...
    def Ry(self, theta):
        ...
        st = np.sin(theta)
        ct = np.cos(theta)
        roty = np.array([[ct, 0.0, -st], [0.0, 1.0, 0.0], [st, 0.0, ct]])
    ...
    def Rz(self, theta):
        ...
        st = np.sin(theta)
        ct = np.cos(theta)
        rotz = np.array([[ct, st, 0.0], [-st, ct, 0.0], [0.0, 0.0, 1.0]])
