...
def compute_mixing_matrix(file: dict, output_mixer_type: str) -> dict[str:float]:
    ...
    for i in range(num_motors):
        T = (
            file['prop_ct']
            * file['air_density']
            * file['prop_diameter'] ** 4
            / (4 * np.pi**2)
            * np.array(motor_orientations[i])
        )

        Q = np.cross(np.array(motor_positions[i]).reshape((1, -1)), T.reshape((1, -1))) + file[
            'prop_cq'
        ] * file['air_density'] * file['prop_diameter'] ** 5 / (4 * np.pi**2) * motor_directions[
            i
        ] * np.array(motor_orientations[i])
    ...
    for i in range(mixing_matrix_inverted.shape[0]):
        for j in range(mixing_matrix_inverted.shape[1]):
            output[output_mixer_type + '_MIXER_' + str(j) + '_' + str(i)] = float(
                np.round(mixing_matrix_inverted[i, j], 4)
            )
...
def format_header(output_formatted: dict, num_motors: int) -> dict:
    ...
    for i in range(num_motors, 10):  # The number of mixer outputs in rosflight firmware
        param_entry = {
            'name': 'PRI_MIXER_OUT_' + str(i),
            'type': 6,
            'value': 0,
        }  # There is no header for the secondary mixer
        output_formatted['root'].append(param_entry)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Script to compute and save mixing matrix parameters'
    )
    parser.add_argument(
        '-f',
        '--param-file',
        type=str,
        default=None,
        help='Location of the frame configuration file. Required',
    )
    parser.add_argument(
        '-o',
        '--output-file',
        type=str,
        default=os.path.join(os.getcwd(), 'mixing_params.yaml'),
        help='Location to save output mixing parameters. Defaults to current working directory',
    )
    parser.add_argument(
        '-t',
        '--output-mixer-type',
        ...
        default='PRI',
        choices=['PRI', 'SEC'],
        help='Whether to compute the primary or secondary matrix parameters. Defaults to primary.',
    )
    parser.add_argument(
        '-a',
        '--append-output',
        default=False,
        action='store_true',
        help='Whether to append the computed string to the file instead of overwriting',
    )
