...
def plotLla():
    ...
    plt.subplot(2, 2, 2)
    plt.plot(
        data.lla['hat'][:, 1] * 180 / np.pi,
        data.lla['hat'][:, 0] * 180.0 / np.pi,
        label=r'$\hat{z}$',
    )
    plt.plot(
        data.lla['bar'][:, 1] * 180 / np.pi,
        data.lla['bar'][:, 0] * 180.0 / np.pi,
        label=r'$\bar{z}$',
    )
    plt.subplot(2, 2, 4)
    ...
    if plotCov:
        plt.plot(
            data.x['t'],
            data.x['ref'][:,] + 2.0 * np.sqrt(data.cov['P'][:, 16, 16]),
            '-k',
            alpha=0.3,
        )
        plt.plot(
...
def plotPosition():
    ...
    for i in range(3):
        ...
        if plotCov:
            plt.plot(
                data.cov['t'],
                data.x['x']['p'][:, i] + 2.0 * np.sqrt(data.cov['P'][:, i, i]),
                '-k',
                alpha=0.3,
            )
            plt.plot(
...
def plotVelocity():
    ...
    for i in range(3):
        ...
        if plotCov:
            plt.plot(
                data.cov['t'],
                data.x['v'][:, i] + 2.0 * np.sqrt(data.cov['P'][:, i + 6, i + 6]),
                '-k',
                alpha=0.3,
            )
            plt.plot(
...
def plotAttitude():
    ...
    for i in range(4):
        ...
        if plotCov and i > 0:
            plt.plot(
                data.cov['t'],
                data.x['x']['q'][:, i] + 2.0 * np.sqrt(data.cov['P'][:, i + 3, i + 3]),
                '-k',
                alpha=0.3,
            )
            plt.plot(
...
def plotEuler():
    ...
    for i in range(3):
        ...
        if plotCov:
            plt.plot(
                data.cov['t'],
                rad2deg * (data.x['euler'][:, i] + 2.0 * np.sqrt(data.cov['P'][:, i + 3, i + 3])),
                '-k',
                alpha=0.3,
            )
            plt.plot(
        ...
        for j in range(2):
            ...
            if j == 0:
                ...
                if plotCov:
                    plt.plot(
                        data.x['t'],
                        data.x['ba'][:, i] + 2.0 * np.sqrt(data.cov['P'][:, i + 9, i + 9]),
                        '-k',
                        alpha=0.3,
                    )
                    plt.plot(
                ...
                if plotCov:
                    plt.plot(
                        data.x['t'],
                        data.x['bg'][:, i] + 2.0 * np.sqrt(data.cov['P'][:, i + 12, i + 12]),
                        '-k',
                        alpha=0.3,
                    )
                    plt.plot(
...
def plotBaroRes():
    ...
    if plotCov:
        plt.plot(
            data.x['t'], data.x['bb'][:,] + 2.0 * np.sqrt(data.cov['P'][:, 15, 15]), '-k', alpha=0.3
        )
        plt.plot(
            data.x['t'], data.x['bb'][:,] - 2.0 * np.sqrt(data.cov['P'][:, 15, 15]), '-k', alpha=0.3
        )
